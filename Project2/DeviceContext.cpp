...
void DeviceContext::setVertexBuffer(TexturedVertexBufferPtr textured_vertex_buffer)
{
	UINT stride = textured_vertex_buffer->m_size_vertex;
	UINT offset = 0;

	m_device_context->IASetVertexBuffers(0, 1, &textured_vertex_buffer->m_buffer, &stride, &offset);
	m_device_context->IASetInputLayout(textured_vertex_buffer->m_layout);
}
