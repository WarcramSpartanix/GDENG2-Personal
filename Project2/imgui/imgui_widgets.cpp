...
// [SECTION] Widgets: ColorEdit, ColorPickerScreen, ColorButton, etc.
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, int frame_padding, const ImVec4& bg_col, const ImVec4& tint_col)
{
    ...
    // Default to using m_texture ID as ID. User can still push string/integer prefixes.
...
// With typical options: Left-click on colored square to open m_color picker. Right-click to open option menu. CTRL-Click over input fields to edit them and TAB to go to next item.
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    // We don't have CPU clipping primitives to clip the CloseButton (until it becomes a m_texture), so need to add an extra draw call (temporary in the case of vertical animation)
...
void ImGui::NextColumn()
{
    ...
    // (which would needlessly attempt to drawUI commands in the wrong channel, then pop or overwrite them),
